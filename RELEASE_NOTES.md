...
- Replaced exact Controller build coupling with semantic `controllerContractVersion: 2` and `start --approve-plan <planDigest>`.
- Added concise `release-result:v1`.
- Removed Dispatcher, legacy Plan v1/v2-direct execution compatibility, manual merge, legacy counters, Completion v1-v3, Controller provenance, runtime lock, executable identity, trust history, and multi-version Job/config readers.
